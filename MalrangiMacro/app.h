...
public:
		...
		CONST CONF_INFO::ACCOUNT_INFO& AccountInfo)
	{
		...
		RETRY_CLICK_MASHUR:
			...
			Sleep(1000);
			imwrite("C:\\" + to_string(rand() % 777777) + ".jpg", Cvw::Capture(CLIENT_ELEM::RECT_CLIENT4));
			...
			else
			{
				...
				try
				{
					Cvw::DoUntilMatchingTemplate(CLIENT_ELEM::RECT_CLIENT4, TargetImageNpcMashur, NONWORK, 30000, 256, 0.99);
				}
				catch (TimeOutException & cwe)
				{
					WriteLog(LOG_LEVEL::FAILURE, cwe.what());
